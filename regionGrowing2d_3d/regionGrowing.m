function [P, J] = regionGrowing(cIM, initPos, thresVal, maxDist, tfMean, tfFillHoles, tfSimplify, rTree, bMask)
    ...
    for j = -1:1
      for k = -1:1
        ...
        if (xv+i > 0  &&  xv+i <= nRow &&...          % within the x-bounds?
           ...
           (rTree(xv+i, yv+j, zv+k) > 0 || bMask(xv+i, yv+j, zv+k) > 0))% of the threshold?
